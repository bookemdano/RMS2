...
namespace Jirzooky
{
    public partial class MainPage : PhoneApplicationPage
    {
        ...
        private void AddIssue(JiraIssue issue, bool bold)
        {
            ...
            lbi.Content = issue.ToString() + " [" + issue.ShortFauxStatus + "]";
            lbi.Tag = issue;
        ...
        private System.Windows.Media.Brush GetBrush(JiraIssue issue)
        {
            ...
            {
                var status = issue.FauxStatus;
                var i = JiraIssue.GetStatusPriority(status);
                if (issue.IsResolved)
            ...
            {
                var delta = JiraIssue.GetStatusPriority(issue.FauxStatus) - JiraIssue.GetStatusPriority(issue.OldFauxStatus);
                if (delta > 0)
                    rv = new SolidColorBrush(Colors.Green);
                else if (delta < 0)
                    rv = new SolidColorBrush(Colors.Red);
